...
namespace jam.CodeBase.GameLoop
{
    public class ResultScreen : BaseView
    {
        ...
        [Title("Weight")] [SerializeField] private Transform _weight;
        ...
        public void SetState(bool isWin)
        {
            ...
            _mainText.text = "";
            _value.text = "";
            _betValue.transform.localScale = Vector3.zero;
        ...
        private async UniTask AnimationFlow(bool isWin, bool isDie)
        {
            ...
            await _mainText.ToType(
                string.Format("He {0}, you {1}", (isDie ? "die" : "survived"), (isWin ? "won" : "lose")), 0.06f);

            var resultMoney = 0f;
            await _betValue.transform.DOScale(2.610547f, 1f).SetEase(Ease.InOutExpo).ToUniTask();
            Debug.LogError("IS WIN: " + isWin + " IS DIE: " + isDie);
            if (isWin)
            {
            ...
            await _value.DOFloatNumber(0, resultMoney, 2f, isWin ? "{0:0}" : "-{0:0}", step: 10).ToUniTask();
